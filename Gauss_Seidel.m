function [x,iter] = Gauss_Seidel(n,p,norma)
%Gauss-Seidel funcion encargada de hallar una solucion al sistema Fx=b
%con una precision de p utilizando el algoritmo de Gauss-Seidel.
...
%Se inicializa el error.
	...
	%Se calcula el nuevo x	
		for i=1:n
			...
			x(i)=(b(i)-sigma)/F(i,i);
			%Se rebica si la diferencia entre el nuevo x_i y el anterior en valor absoluto es mayor que le maximo
			if abs(x(i)-ant)>maxi
	...
	while(epsilon>=p)
		iter=iter+1;
	%Es necesario guardar todo el vectro anterior.
		ant = x;
		for i=1:n
			sigma = 0;
			for j=1:n
